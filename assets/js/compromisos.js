...
// Versión corregida: eliminar compromisos funciona con IDs estables
// ======================================
...
function toMunicipio(v) {
  return norm(v || "Municipio 1").replace(/^Comuna/i, "Municipio");
}
...
function getReuniones() {
  ...
  return arr.map(r => ({
    ...r,
    municipio: toMunicipio(r.municipio || r.comuna)
  }));
}

// IMPORTANTE:
// Esta función ahora garantiza IDs estables.
// Si un compromiso no tiene id, se lo asigna y lo guarda inmediatamente.
function getCompromisos() {
  ...
  const normalizados = arr.map((c) => {
    const copia = normalizarCompromiso(c);

    if (!c.id) huboCambios = true;
    if (c.comuna && !c.municipio) huboCambios = true;

    return copia;
  });

  if (huboCambios) {
    saveCompromisos(normalizados);
  }
...
// Ahora pueden eliminar admin, gerencia, coordinador y supervisor.
function puedeEliminarCompromisos(sesion) {
  return isFullAccess(sesion?.rol);
}
...
function poblarSelectReunion(municipio, liderKeyValue) {
  ...
  if (liderKeyValue?.startsWith("name:")) {
    liderNombre = liderKeyValue.slice(5);
  }

  if (liderKeyValue?.startsWith("id:")) {
    const idVal = liderKeyValue.slice(3);
    liderNombre = cargarLideresDeMunicipio(municipio)
      .find(l => String(l.id) === String(idVal))?.nombre || "";
  }
...
function badgeEstado(e) {
  ...
  if (v.includes("cumpl")) {
    return `<span style="${base} background:#dcfce7; color:#166534;">Cumplido</span>`;
  }

  if (v.includes("gestion")) {
    return `<span style="${base} background:#dbeafe; color:#1e40af;">En gestión</span>`;
  }
...
function normalizarCompromiso(c) {
  return {
    ...
    potencialVotos: (c.potencialVotos !== undefined && c.potencialVotos !== null)
      ? parsePotencial(c.potencialVotos)
      : null,
...
function getListaMunicipiosDesdeDatos() {
  ...
  if (!municipios.length) {
    return Array.from({ length: 12 }, (_, i) => `Municipio ${i + 1}`);
  }
...
function renderTabla(sesion, municipioBase, filtroMunicipio) {
  ...
  const puedeEliminar = puedeEliminarCompromisos(sesion);
  ...
  tbody.innerHTML = data.map(c => {
    ...
    const potTxt = (c.potencialVotos === null || c.potencialVotos === undefined)
      ? "—"
      : String(c.potencialVotos);
...
function exportExcel(sesion, municipioBase, filtroMunicipio) {
  ...
  const rows = data.map(c => ({
    ...
    Responsable: c.responsable || "",
    "Potencial de votos": (c.potencialVotos === null || c.potencialVotos === undefined)
      ? ""
      : c.potencialVotos,
    Estado: c.estado || "",
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  selLider?.addEventListener("change", () => {
    poblarSelectReunion(municipioBase, selLider.value || "");
  });
  ...
  tbody?.addEventListener("click", (ev) => {
    ...
    if (!puedeEliminarCompromisos(sesion)) {
    ...
    const nuevo = arr.filter(x => String(x.id) !== String(compId));
  ...
  tbody?.addEventListener("change", (ev) => {
    ...
    const arr = getCompromisos();
    const idx = arr.findIndex(x => String(x.id) === String(compId));
  ...
  document.getElementById("comp-form")?.addEventListener("submit", (e) => {
    ...
    if (!liderKey || !tipoCompromiso || !estado || !prioridad || !fecha) {
      alert("Por favor completa líder, tipo de compromiso, estado, prioridad y fecha.");
      return;
    }
